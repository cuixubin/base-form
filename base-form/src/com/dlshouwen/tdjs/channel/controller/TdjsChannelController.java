 ...
 */
package com.dlshouwen.tdjs.channel.controller;
 ...
 */
@Controller
@RequestMapping("/tdjs/tdjsChannel/channel")
public class TdjsChannelController {

    //根路径
    private String basePath = "tdjs/channel/";
    ...
    public String goChannelPage(HttpServletRequest request, Model model) throws Exception {
        ...
        List<Map<String, Object>> channelList = dao.getRecordsByIdentityAndTeamId(user.getIdentity(), user.getTeam_id());
    ...
    public AjaxResponse addChannel(@Valid Channel channel, BindingResult bindingResult,
        ...
        //通知前端跳转路径
        Map map = new HashMap();
        map.put("URL", "tdjs/tdjsChannel/channel");
    ...
    public String editChannel(@PathVariable String channelId, HttpServletRequest request, Model model) throws Exception {
        Channel channel = dao.getChannelById(channelId);
        model.addAttribute("channel", channel);
...
//		记录操作日志
     ...
     * @throws Exception 
     */
    @RequestMapping(value = "/{teamId}/selectChannel", method = RequestMethod.GET)
    public String selectChannelByTeamId(@PathVariable String teamId,
            HttpServletRequest request, Model model) throws Exception {
//		获取栏目树传递到前台
        List<Map<String, Object>> channelList = dao.getChannelListByTeamId(teamId);
