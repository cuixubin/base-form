    ...
    <head>
        ...
        <script type="text/javascript">
            var gridColumns = [
                ...
                {id: 'id', title: '编号', type: 'string', fastQuery: true, fastQueryType: 'lk'},
                ...
                {id: 'status', title: '状态', type: 'string', codeTable: code_table_info['review_status'], columnClass: 'text-center', fastQuery: true, fastQueryType: 'eq', resolution: function (value, record, column, grid, dataNo, columnNo) {
                        if (value == 0) {
            ...
            var gridOption = {
                loadURL: '${pageContext.request.contextPath }/jspc/zjpc/zjpc/admin/list',
...
//                    urls += 'wzgl/article/article/-/edit,';
            ...
            function reviewUserSingle(id) {
                var url = '${pageContext.request.contextPath }/jspc/zjpc/zjpc/' + id + '/adminEdit';
                sw.showProcessBar();
            ...
            //删除单条数据
            function deleteUserSingle(id) {
                if (!confirm('确定删除此评审信息吗？')) {
                    return;
                }
                var url = '${pageContext.request.contextPath }/jspc/zjpc/zjpc/delete';
            ...
            function deleteReview() {
                var recordObj = grid.getCheckedRecords();
                if (recordObj.length == 0) {
                    sw.toast('请选择要删除的评审信息！', 'warning', 3000);
                    return false;
                }
                if (!confirm('确定删除选中评审信息吗？')) {
                    return;
                }
                var reviewIds = '';
                for (var i = 0; i < recordObj.length; i++) {
                    reviewIds += recordObj[i].id + ((i != recordObj.length - 1) ? ',' : '');
                }
                var url = '${pageContext.request.contextPath }/jspc/zjpc/zjpc/delete';
                var params = new Object();
                params.ids = reviewIds;
                sw.showProcessBar();
                sw.ajaxSubmitRefreshGrid(url, params, grid);
            }
            //初始化绑定方法
            $(function () {
                ...
                $('#delete_review').bind('click', deleteReview);
            });
        </script>
    </head>
    <body>
        ...
        <div class="main-container">
            <ul class="breadcrumb">
                ...
                <li class="active"><a href="javascript:void(0);"><i class="fa fa-legal"></i>专家评测(管理员)</a></li>
            </ul>
            <div class="panel panel-default">
                <div class="panel-heading"><i class="fa fa-bell"></i>专家评测(管理员)</div>
                <div class="panel-operation">
                    <div class="btn-group">
                        <sw:button accesskey="M" id="expert_review" value="评审 (M)" cssClass="btn btn-primary" limit="jspc/zjpc/zjpc/review" icon="edit" />
                        <sw:button accesskey="W" id="delete_review" value="删除 (W)" cssClass="btn btn-danger" limit="jspc/zjpc/zjpc/delete" icon="trash-o" />
                    </div>
                </div>
