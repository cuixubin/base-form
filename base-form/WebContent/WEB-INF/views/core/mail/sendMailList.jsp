...
<html>
<head>
	<title>${applicationScope.__SYSTEM_ATTRIBUTE__.page_title.content } - 发件箱</title>
	<base target="_self" />
	...
	var gridColumns = [
		{id:'operation', title:'操作', columnClass:'text-center', columnStyle:'width:140px;', hideType:'xs', extra:false, 'export':false, print:false, advanceQuery:false, resolution:function(value, record, column, grid, dataNo, columnNo){
			...
			content += '&nbsp;&nbsp;';
			content += '<button type="button" class="btn btn-xs btn-icon btn-danger" title="删除" onclick="deleteMailForSendSingle(\''+record.mail_id+'\')"><i class="fa fa-trash-o"></i></button>';
			return content;
		}},
		{id:'title', title:'邮件主题', type:'string', columnClass:'text-center', hideType:'', fastQuery:true, fastQueryType:'lk' },
		{id:'receiver_name', title:'收件人', columnClass:'text-center', hideType:'xs|sm', fastQuery:true, fastQueryType:'lk', resolution:function(value, record, column, grid, dataNo, columnNo){
		...
		{id:'create_time', title:'创建时间', type:'date', format:'yyyy-MM-dd hh:mm:ss', columnClass:'text-center', hideType:'xs|sm', fastQuery:true, fastQueryType:'range' },
		{id:'send_time', title:'发送时间', type:'date', format:'yyyy-MM-dd hh:mm:ss', otype:'string', oformat:'yyyy-MM-dd hh:mm:ss', columnClass:'text-center', hideType:'xs', fastQuery:true, fastQueryType:'range' }
	];
	var gridOption={
		loadURL : '${pageContext.request.contextPath }/core/mail/send/list',
		functionCode : 'CORE_MAIL_SEND',
		check:true,
		exportFileName : '发件箱列表',
		columns : gridColumns,
	...
	function deleteMailForSend(){
		...
		url = '${pageContext.request.contextPath }/core/mail/delete/send';
	...
	//删除操作
	function deleteMailForSendSingle(mailId){
	...
	$(function(){
		...
		$('#delete_mail_for_send').bind('click', deleteMailForSend);
	});
	</script>
</head>
<body>
	...
	<div class="main-container">
		<ul class="breadcrumb">
			...
			<li class="active"><a href="javascript:void(0);"><i class="fa fa-paper-plane-o"></i>发件箱</a></li>
		</ul>
		<div class="panel panel-default">
			<div class="panel-heading"><i class="fa fa-paper-plane-o"></i>发件箱</div>
			<div class="panel-operation">
				<div class="btn-group">
					...
					<sw:button accesskey="W" id="delete_mail_for_send" value="删除 (W)" cssClass="btn btn-danger" icon="trash-o" />
				</div>
			</div>
