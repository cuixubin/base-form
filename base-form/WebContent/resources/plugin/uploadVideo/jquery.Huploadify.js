...
$.fn.Huploadify = function(opts){
	var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-progress" style="float:left"><div class="uploadify-progress-bar"></div></div><span class="uploadbtn">上传</span><span class="delfilebtn" style="display:none">删除</span></div>';
	var defaults = {
	...
	this.each(function(){
		...
		inputStr += '"/>';
		inputStr += '<a id="file_upload_'+instanceNumber+'-button" href="javascript:void(0)" class="uploadify-button" style="margin-left:0px">';
	  ...
	  fileObj = {
				  ...
				  for(var i=0,len=files.length;i<len;i++){
			  		...
			  		if(parseInt(formatFileSize(thisFile.size,true))>option.fileSizeLimit){
						...
						if($.inArray(thisFile.name.split('.').pop().toLowerCase(),typeArray)>=0 || $.inArray('*', typeArray)>=0){
							...
							alert('文件'+thisFile.name+'类型不允许！请上传以下格式的视频文件：'+defaults.fileTypeExts);
						}  	
					}	
				}
			  return arr;  	
		  ...
		  funSelect: function(files){
				for(var i=0,len=files.length;i<len;i++){
					...
					if(option.showUploadedPercent){
						var percentText = '<span class="up_percent" style="float:left">'+initUppercent+'</span>';
					...
					if(option.auto){
						...
						//如果配置非自动上传，绑定上传事件
					 	$html.find('.uploadbtn').bind('click',(function(file){
					 			return function(){fileObj.funUploadFile(file);}
					 		})(file));
					}
					//为删除文件按钮绑定删除文件事件
			 		$html.find('.delfilebtn').bind('click',(function(file){
		  ...
		  init: function() {	  
			  ...
			  _this.find('.uploadify-button').bind('click',function(){
	...
	var returnObj =  {
		...
		disable : function(instanceID){
			...
			parent.find('.uploadify-button').css('background-color','#888').unbind('click');
		},
		ennable : function(instanceID){
			//点击上传按钮时触发file的click事件
			var parent = instanceID ? $('file_upload_'+instanceID+'-button') : $('body');
		  	parent.find('.uploadify-button').css('background-color','#707070').bind('click',function(){
