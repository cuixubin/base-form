...
(function($) {
	...
	$.fn.dlshouwen.grid = {
		...
		init : function(options) {
			...
			for(var i=0; i<options.columns.length; i++){
				//初始化列参数
				elementFormat[options.columns[i].id] = options.columns[i].format;  
			}
			/**
			 * 2. 定义对象
			 */
			var gridObject = {
							...
							success:function(pager){
								...
								gridReflectionObj.hideProcessBar(function(){
									...
									gridReflectionObj.pager.pageCount = pager.pageCount;
                                                                        //返回sql信息
                                                                        gridReflectionObj.pager.sqlString = pager.sqlString;
                                                                        gridReflectionObj.pager.params = pager.params;
									//构建表格、工具条
									gridReflectionObj.constructGrid();
									gridReflectionObj.constructGridPageBar();
								});
				...
				constructGrid : function(){
					...
					if(gridReflectionObj.option.check){
						$('#dlshouwen_grid_'+gridId+'_check').click(function(e){
							$('input[id*=dlshouwen_grid_'+gridId+'_check_]').attr('checked', this.checked);
							if(gridReflectionObj.option.onCheck){
				...
				constructGridPageBar : function(){
					...
					operations.className = 'pagination dlshouwen-grid-pager-button';
				...
				constructGridToolBar : function(){
					...
					var gridReflectionObj = this;
					//清空工具条
					$('#'+gridReflectionObj.option.toolbarContainer).html('<span class="pagination dlshouwen-grid-tools"></span><span class="dlshouwen-grid-pager"></span>');
					//如果为空则不做任何操作
				...
				fastQuery : function(){
					...
					content += '	<form class="form-horizontal" id="fastQueryForm_'+gridReflectionObj.option.id+'">';
							...
							if(fastQueryParameters[key]&&fastQueryParameters[key]!=null&&fastQueryParameters[key]!=''){
								...
								if($.fn.dlshouwen.grid.tools.startsWith(key, 'gt_')){
									...
									if(value instanceof Date){
										if((value.getTime()-$.fn.dlshouwen.grid.tools.toDate(fastQueryParameters[key], gridReflectionObj.elementFormat[key.replace('gt_', '')]).getTime())<=0){
											isShow = false;
											continue;
										}
									}
								...
								if($.fn.dlshouwen.grid.tools.startsWith(key, 'ge_')){
									...
									//日期比较
									if(value instanceof Date){
										if((value.getTime()-$.fn.dlshouwen.grid.tools.toDate(fastQueryParameters[key], gridReflectionObj.elementFormat[key.replace('ge_', '')]).getTime())<0){
											isShow = false;
											continue;
										}
									}
								...
								if($.fn.dlshouwen.grid.tools.startsWith(key, 'lt_')){
									...
									//日期比较
									if(value instanceof Date){
										if((value.getTime()-$.fn.dlshouwen.grid.tools.toDate(fastQueryParameters[key], gridReflectionObj.elementFormat[key.replace('lt_', '')]).getTime())>=0){
											isShow = false;
											continue;
										}
									}
								...
								if($.fn.dlshouwen.grid.tools.startsWith(key, 'le_')){
									...
									//日期比较
									if(value instanceof Date){
										if((value.getTime()-$.fn.dlshouwen.grid.tools.toDate(fastQueryParameters[key], gridReflectionObj.elementFormat[key.replace('le_', '')]).getTime())>0){
											isShow = false;
											continue;
										}
									}
				...
				advanceQuery : function(){
					...
					//加载查询内容
					var content = '';
					content += $.fn.dlshouwen.grid.tools.getWindowStart('advance_query_'+gridReflectionObj.option.id, $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.title, 'lg');
					content += '<div class="modal-body advance-query">';
					content += '	<div class="row">';
					content += '		<div class="col-lg-3 col-md-3 col-sm-3">';
					content += '			<div class="panel-group accordion" id="accordion">';
					content += '				<div class="panel panel-default">';
					content += '					<div class="panel-heading" data-toggle="collapse" data-toggle="collapse" data-parent="#accordion" href="#advance_query_list_'+gridReflectionObj.option.id+'">';
					content += '						<strong><i class="fa fa-folder-o"></i>查询方案</strong>';
					content += '					</div>';
					content += '					<div id="advance_query_list_'+gridReflectionObj.option.id+'" class="panel-collapse collapse list-group in" style="min-height:120px;"></div>';
					content += '				</div>';
					content += '			</div>';
					content += '		</div>';
					content += '	<div class="col-lg-9 col-md-9 col-sm-9">';
					...
					content += '			<div class="panel panel-default">';
					content += '				<input type="hidden" id="advance_query_id_'+gridReflectionObj.option.id+'" name="advance_query_id_'+gridReflectionObj.option.id+'" />';
					content += '				<div class="form-horizontal" style="padding:10px;">';
					content += '					<div class="form-group">';
					...
					content += '							<input type="text" class="form-control" id="advance_query_name_'+gridReflectionObj.option.id+'" name="advance_query_name_'+gridReflectionObj.option.id+'" placeholder="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.advanceQueryNamePlaceHoder+'">';
					content += '						</div>';
					content += '					</div>';
					content += '					<div class="form-group">';
					...
					content += '							<button id="add_advance_query_'+gridReflectionObj.option.id+'" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.addAdvanceQueryTitle+'" class="btn btn-primary">';
					content += '								'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.addAdvanceQuery;
					content += '							</button>';
					content += '							<button id="edit_advance_query_'+gridReflectionObj.option.id+'" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.editAdvanceQueryTitle+'" class="btn btn-primary">';
					content += '								'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.editAdvanceQuery;
					content += '							</button>';
					content += '							<button id="copy_advance_query_'+gridReflectionObj.option.id+'" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.copyAdvanceQueryTitle+'" class="btn btn-warning">';
					content += '								'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.copyAdvanceQuery;
					content += '							</button>';
					content += '							<button id="delete_advance_query_'+gridReflectionObj.option.id+'" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.plan.buttons.deleteAdvanceQueryTitle+'" class="btn btn-danger">';
					...
					content += '					<button id="insert_condition_tr_'+gridReflectionObj.option.id+'" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.buttons.insertTitle+'" class="btn btn-primary">';
					content += '						'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.buttons.insert;
					content += '					</button>';
					content += '					<button id="clear_condition_tr_'+gridReflectionObj.option.id+'" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.buttons.clearTitle+'" class="btn btn-danger">';
					...
					content += '					<table id="condition_table_'+gridReflectionObj.option.id+'" class="table table-bordered table-striped table-hover table-condition table-center" style="min-width:800px">';
					content += '						<tr>';
					...
					content += '			<div class="panel panel-default">';
					content += '				<div class="form-horizontal text-right advance-query-buttons">';
					content += '					<button id="insert_sort_tr_'+gridReflectionObj.option.id+'" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.buttons.insertTitle+'" class="btn btn-primary">';
					content += '						'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.buttons.insert;
					content += '					</button>';
					content += '					<button id="clear_sort_tr_'+gridReflectionObj.option.id+'" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.buttons.clearTitle+'" class="btn btn-danger">';
					content += '						'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.buttons.clear;
					content += '					</button>';
					content += '				</div>';
					content += '				<div class="advance-query-table-container">';
					content += '					<table id="sort_table_'+gridReflectionObj.option.id+'" class="table table-bordered table-striped table-hover table-condition table-center" style="width:400px">';
					content += '						<tr>';
					content += '							<th style="width:50px;">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.sequence+'</th>';
					content += '							<th style="width:100px;">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.operation+'</th>';
					content += '							<th>'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.field+'</th>';
					content += '							<th style="width:80px;">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.logic+'</th>';
					content += '						</tr>';
					content += '					</table>';
					content += '				</div>';
					content += '				<div class="clearfix"></div>';
					...
					content += '</div>';
					var buttons = '';
					buttons += '<button type="button" class="btn btn-primary" id="do_advance_query_'+gridReflectionObj.option.id+'">';
					buttons += '	'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.buttons.query;
					buttons += '</button>';
					content += $.fn.dlshouwen.grid.tools.getWindowEnd($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].buttons.close, buttons);
					$('body').append(content);
					//加载查询方案列表
					gridReflectionObj.loadAdvanceQueryList();
					//新增高级查询方案
					$('#add_advance_query_'+gridReflectionObj.option.id).click(function(){
						gridReflectionObj.addAdvanceQuery();
					});
					//编辑高级查询方案
					$('#edit_advance_query_'+gridReflectionObj.option.id).click(function(){
						gridReflectionObj.editAdvanceQuery();
					});
					//复制高级查询方案
					$('#copy_advance_query_'+gridReflectionObj.option.id).click(function(){
						gridReflectionObj.copyAdvanceQuery();
					});
					//删除高级查询方案
					$('#delete_advance_query_'+gridReflectionObj.option.id).click(function(){
						gridReflectionObj.deleteAdvanceQuery();
					});
					//新增一行查询条件
					$('#insert_condition_tr_'+gridReflectionObj.option.id).click(function(){
						gridReflectionObj.insertConditionTr();
					});
					//清空查询条件
					$('#clear_condition_tr_'+gridReflectionObj.option.id).click(function(){
						gridReflectionObj.clearConditionTr();
					});
					//新增一行排序条件
					$('#insert_sort_tr_'+gridReflectionObj.option.id).click(function(){
						gridReflectionObj.insertSortTr();
					});
					//清空排序条件
					$('#clear_sort_tr_'+gridReflectionObj.option.id).click(function(){
						gridReflectionObj.clearSortTr();
					});
					//绑定方法：执行高级查询
					$('#do_advance_query_'+gridReflectionObj.option.id).click(function(){
				...
				//高级查询方案加载
				loadAdvanceQueryList : function(){
					//定义表格对象映像
					var gridReflectionObj = this;
					//获取高级查询方案
					var url = path+'/core/base/extra/advance_query/list';
					var params = new Object();
					params.functionCode = gridReflectionObj.option.functionCode;
					sw.ajaxSubmit(url, params, function(datas){
						var content = '';
						for(var i=0; i<datas.length; i++){
							var advanceQuery = datas[i];
							content += '<a id="advance_query_'+gridReflectionObj.option.id+'_'+advanceQuery.advance_query_id+'" class="list-group-item" href="javascript:void(0);">';
							content += '	<i class="fa fa-file-text-o"></i>'+advanceQuery.advance_query_name;
							content += '</a>';
						}
						$('#advance_query_list_'+gridReflectionObj.option.id).html(content);
						//绑定方法
						$('a[id*=advance_query_'+gridReflectionObj.option.id+'_]').click(function(){
							var advanceQueryId = this.id.replace('advance_query_'+gridReflectionObj.option.id+'_', '');
							gridReflectionObj.loadAdvanceQueryDeatil(advanceQueryId);
						});
					});
				},
				//加载方案
				loadAdvanceQueryDeatil : function(advanceQueryId){
					//定义表格对象映像
					var gridReflectionObj = this;
					//清空所有行
					gridReflectionObj.clearConditionTr();
					gridReflectionObj.clearSortTr();
					//获取快速查询方案及数据
					var url = path+'/core/base/extra/advance_query/info/data';
					var params = new Object();
					params.advanceQueryId = advanceQueryId;
					sw.ajaxSubmit(url, params, function(data){
						//获取快速查询方案
						var _advanceQuery = data;
						//快速查询方案赋值
						$('#advance_query_id_'+gridReflectionObj.option.id).val(_advanceQuery.advance_query_id);
						$('#advance_query_name_'+gridReflectionObj.option.id).val(_advanceQuery.advance_query_name);
						$('#remark_'+gridReflectionObj.option.id).val(_advanceQuery.remark);
						//获取快速查询数据
						var advanceQueryConditionList = _advanceQuery.advanceQueryConditionList;
						//遍历快速查询数据处理反显
						for(var i=0; i<advanceQueryConditionList.length; i++){
							var dataType = advanceQueryConditionList[i].data_type;
							var leftParentheses = advanceQueryConditionList[i].left_parentheses;
							var conditionField = advanceQueryConditionList[i].condition_field;
							var conditionType = advanceQueryConditionList[i].condition_type;
							var conditionValue = advanceQueryConditionList[i].condition_value;
							var rightParentheses = advanceQueryConditionList[i].right_parentheses;
							var logic = advanceQueryConditionList[i].logic;
							var sequenceCondition = gridReflectionObj.insertConditionTr();
							$('#data_type'+gridReflectionObj.option.id+'_'+sequenceCondition).val(dataType);
							$('#left_parentheses_'+gridReflectionObj.option.id+'_'+sequenceCondition).val(leftParentheses);
							$('#condition_field_'+gridReflectionObj.option.id+'_'+sequenceCondition).val(conditionField);
							$('#condition_field_'+gridReflectionObj.option.id+'_'+sequenceCondition).change();
							$('#condition_type_'+gridReflectionObj.option.id+'_'+sequenceCondition).val(conditionType);
							$('#condition_value_'+gridReflectionObj.option.id+'_'+sequenceCondition).val(conditionValue);
							$('#right_parentheses_'+gridReflectionObj.option.id+'_'+sequenceCondition).val(rightParentheses);
							$('#logic_'+gridReflectionObj.option.id+'_'+sequenceCondition).val(logic);
						}
						//获取快速排序数据
						var advanceQuerySortList = _advanceQuery.advanceQuerySortList;
						//遍历快速排序数据处理反显
						for(var i=0; i<advanceQuerySortList.length; i++){
							var sortField = advanceQuerySortList[i].sort_field;
							var sortLogic = advanceQuerySortList[i].sort_logic;
							var sequenceSort = gridReflectionObj.insertSortTr();
							$('#sort_field_'+gridReflectionObj.option.id+'_'+sequenceSort).val(sortField);
							$('#sort_logic_'+gridReflectionObj.option.id+'_'+sequenceSort).val(sortLogic);
						}
					});
				},
				//新建查询方案
				addAdvanceQuery : function(){
					//定义表格对象映像
					var gridReflectionObj = this;
					//判断主方案信息是否有误
					var advanceQueryName = $('#advance_query_name_'+gridReflectionObj.option.id).val();
					if(advanceQueryName.trim()==''){
						$.fn.dlshouwen.grid.tools.toast('方案名称必须填写。', 'warning', 3000);
						$('#advance_query_name_'+gridReflectionObj.option.id).focus();
						return;
					}
					if(advanceQueryName.lengthb>40){
						$.fn.dlshouwen.grid.tools.toast('方案名称的长度仅限40个字符。', 'warning', 3000);
						$('#advance_query_name_'+gridReflectionObj.option.id).focus();
						return;
					}
					//拼接方案对象
					var _advanceQuery = new Object();
					_advanceQuery.advance_query_name = advanceQueryName;
					_advanceQuery.function_code = gridReflectionObj.option.functionCode;
					_advanceQuery.remark = $('#remark_'+gridReflectionObj.option.id).val();
					//判断并拼接查询方案
					var advanceQueryConditionList = new Array();
					var pass = true;
					$('#condition_table_'+gridReflectionObj.option.id+' tr[id*=tr_'+gridReflectionObj.option.id+'_]').each(function(){
						var id = this.id.replace('tr_'+gridReflectionObj.option.id+'_', '');
						var seq = $('#seq_td_'+gridReflectionObj.option.id+'_'+id).html();
						var dataType = $('#data_type_'+gridReflectionObj.option.id+'_'+id).val();
						var leftParentheses = $('#left_parentheses_'+gridReflectionObj.option.id+'_'+id).val();
						var conditionField = $('#condition_field_'+gridReflectionObj.option.id+'_'+id).val();
						var conditionType = $('#condition_type_'+gridReflectionObj.option.id+'_'+id).val();
						var conditionValue = $('#condition_value_'+gridReflectionObj.option.id+'_'+id).val();
						var rightParentheses = $('#right_parentheses_'+gridReflectionObj.option.id+'_'+id).val();
						var logic = $('#logic_'+gridReflectionObj.option.id+'_'+id).val();
						if(conditionField==''){
							$.fn.dlshouwen.grid.tools.toast('查询条件中第'+seq+'行的字段信息必须选择！', 'warning', 3000);
							pass = false;
							return false;
						}
						if(conditionType==''){
							$.fn.dlshouwen.grid.tools.toast('查询条件中第'+seq+'行的条件信息必须选择！', 'warning', 3000);
							pass = false;
							return false;
						}
						var advanceQueryCondition = new Object();
						advanceQueryCondition.data_type = dataType;
						advanceQueryCondition.left_parentheses = leftParentheses;
						advanceQueryCondition.condition_field = conditionField;
						advanceQueryCondition.condition_type = conditionType;
						advanceQueryCondition.condition_value = conditionValue;
						advanceQueryCondition.right_parentheses = rightParentheses;
					...
					_advanceQuery.advanceQueryConditionList = advanceQueryConditionList;
					//判断并拼接排序方案
					var advanceQuerySortList = new Array();
					var pass = true;
					$('#sort_table_'+gridReflectionObj.option.id+' tr[id*=tr_'+gridReflectionObj.option.id+'_]').each(function(){
						var id = this.id.replace('tr_'+gridReflectionObj.option.id+'_', '');
						var seq = $('#seq_td_'+gridReflectionObj.option.id+'_'+id).html();
						var sortField = $('#sort_field_'+gridReflectionObj.option.id+'_'+id).val();
						var sortLogic = $('#sort_logic_'+gridReflectionObj.option.id+'_'+id).val();
						if(sortField==''){
							$.fn.dlshouwen.grid.tools.toast('排序条件中第'+seq+'行的字段信息必须选择！', 'warning', 3000);
							pass = false;
							return false;
						}
						if(sortLogic==''){
							$.fn.dlshouwen.grid.tools.toast('排序条件中第'+seq+'行的排序逻辑必须选择！', 'warning', 3000);
							pass = false;
							return false;
						}
						var advanceQuerySort = new Object();
						advanceQuerySort.sort_field = sortField;
						advanceQuerySort.sort_logic = sortLogic;
						advanceQuerySortList.push(advanceQuerySort);
					});
					if(!pass){
						return;
					}
					_advanceQuery.advanceQuerySortList = advanceQuerySortList;
					//执行新增操作
					if(!confirm('您确定新增此查询方案？')){
						return;
					}
					var url = path+'/core/base/extra/advance_query/add';
					sw.ajaxSubmitJSON(url, _advanceQuery, function(data){
				...
				//编辑查询方案
				editAdvanceQuery : function(){
					//定义表格对象映像
					var gridReflectionObj = this;
					//判断当前是否打开了方案
					var advanceQueryId = $('#advance_query_id_'+gridReflectionObj.option.id).val();
					if(advanceQueryId.trim()==''){
						$.fn.dlshouwen.grid.tools.toast('您未选择任何方案，无法执行编辑操作。', 'warning', 3000);
						return;
					}
					//判断主方案信息是否有误
					var advanceQueryName = $('#advance_query_name_'+gridReflectionObj.option.id).val();
					...
					//拼接方案对象
					var _advanceQuery = new Object();
					_advanceQuery.advance_query_id = advanceQueryId;
					_advanceQuery.advance_query_name = advanceQueryName;
					_advanceQuery.function_code = gridReflectionObj.option.functionCode;
					_advanceQuery.remark = $('#remark_'+gridReflectionObj.option.id).val();
					//判断并拼接查询方案
					...
					_advanceQuery.advanceQuerySortList = advanceQuerySortList;
					//执行编辑操作
					if(!confirm('您确定编辑此查询方案？')){
						return;
					}
					var url = path+'/core/base/extra/advance_query/edit';
					sw.ajaxSubmitJSON(url, _advanceQuery, function(data){
						sw.ajaxSuccessCallback(data, function(){
							gridReflectionObj.loadAdvanceQueryList();
						});
				...
				//删除方案
				deleteAdvanceQuery : function(){
					//定义表格对象映像
					var gridReflectionObj = this;
					//判断当前是否打开了方案
					var advanceQueryId = $('#advance_query_id_'+gridReflectionObj.option.id).val();
					if(advanceQueryId.trim()==''){
						$.fn.dlshouwen.grid.tools.toast('您未选择任何方案，无法执行删除操作。', 'warning', 3000);
						return;
					}
					//执行删除操作
					if(!confirm('您确定删除此查询方案？')){
						return;
					}
					var url = path+'/core/base/extra/advance_query/delete';
					var params = new Object();
					params.advanceQueryId = advanceQueryId;
					sw.ajaxSubmit(url, params, function(data){
						sw.ajaxSuccessCallback(data, function(){
							$('#advance_query_id_'+gridReflectionObj.option.id).val('');
							$('#advance_query_name_'+gridReflectionObj.option.id).val('');
							$('#remark_'+gridReflectionObj.option.id).val('');
							gridReflectionObj.loadAdvanceQueryList();
						});
					});
				},
				//复制方案
				copyAdvanceQuery : function(){
					//定义表格对象映像
					var gridReflectionObj = this;
					//判断当前是否打开了方案
					var advanceQueryId = $('#advance_query_id_'+gridReflectionObj.option.id).val();
					if(advanceQueryId.trim()==''){
						$.fn.dlshouwen.grid.tools.toast('您未选择任何方案，无法执行复制操作。', 'warning', 3000);
						return;
					}
					//执行删除操作
					if(!confirm('您确定复制此查询方案？')){
						return;
					}
					var url = path+'/core/base/extra/advance_query/copy';
					var params = new Object();
					params.advanceQueryId = advanceQueryId;
					sw.ajaxSubmit(url, params, function(data){
						sw.ajaxSuccessCallback(data, function(){
				...
				doAdvanceQuery : function(){
					...
					$('#condition_table_'+gridReflectionObj.option.id+' tr[id*=tr_'+gridReflectionObj.option.id+'_]').each(function(){
						var id = this.id.replace('tr_'+gridReflectionObj.option.id+'_', '');
						var seq = $('#condition_table_'+gridReflectionObj.option.id+' #seq_td_'+gridReflectionObj.option.id+'_'+id).html();
						var leftParentheses = $('#condition_table_'+gridReflectionObj.option.id+' #left_parentheses_'+gridReflectionObj.option.id+'_'+id).val();
						var conditionField = $('#condition_table_'+gridReflectionObj.option.id+' #condition_field_'+gridReflectionObj.option.id+'_'+id).val();
						var conditionType = $('#condition_table_'+gridReflectionObj.option.id+' #condition_type_'+gridReflectionObj.option.id+'_'+id).val();
						var conditionValue = $('#condition_table_'+gridReflectionObj.option.id+' #condition_value_'+gridReflectionObj.option.id+'_'+id).val();
						var rightParentheses = $('#condition_table_'+gridReflectionObj.option.id+' #right_parentheses_'+gridReflectionObj.option.id+'_'+id).val();
						var logic = $('#condition_table_'+gridReflectionObj.option.id+' #logic_'+gridReflectionObj.option.id+'_'+id).val();
						if(conditionField==''){
					...
					$('#sort_table_'+gridReflectionObj.option.id+' tr[id*=tr_'+gridReflectionObj.option.id+'_]').each(function(){
						var id = this.id.replace('tr_'+gridReflectionObj.option.id+'_', '');
						var seq = $('#sort_table_'+gridReflectionObj.option.id+' #seq_td_'+gridReflectionObj.option.id+'_'+id).html();
						var sortField = $('#sort_table_'+gridReflectionObj.option.id+' #sort_field_'+gridReflectionObj.option.id+'_'+id).val();
						var sortLogic = $('#sort_table_'+gridReflectionObj.option.id+' #sort_logic_'+gridReflectionObj.option.id+'_'+id).val();
						if(sortField==''){
						...
						if(advanceQueryParameter.advanceQueryConditions&&advanceQueryParameter.advanceQueryConditions.length>0){
							...
							for(var j=0; j<advanceQueryParameter.advanceQueryConditions.length; j++){
								...
								if(advanceQueryCondition.condition=='0'){
									condition += 'record["'+advanceQueryCondition.condition_field+'"]=="'+advanceQueryCondition.condition_value+'"';
								}
								if(advanceQueryCondition.condition=='1'){
									condition += 'record["'+advanceQueryCondition.condition_field+'"]!="'+advanceQueryCondition.condition_value+'"';
								}
								if(advanceQueryCondition.condition=='2'){
									condition += 'record["'+advanceQueryCondition.condition_field+'"].indexOf("'+advanceQueryCondition.condition_value+'")!=-1';
								}
								if(advanceQueryCondition.condition=='3'){
									condition += '$.fn.dlshouwen.grid.tools.startsWith(record["'+advanceQueryCondition.condition_field+'"], "'+advanceQueryCondition.condition_value+'")';
								}
								if(advanceQueryCondition.condition=='4'){
									condition += '$.fn.dlshouwen.grid.tools.endsWith(record["'+advanceQueryCondition.condition_field+'"], "'+advanceQueryCondition.condition_value+'")';
								}
								if(advanceQueryCondition.condition=='5'){
									var value = record[advanceQueryCondition.condition_field];
									//日期比较
									if(value instanceof Date){
										condition += '(record["'+advanceQueryCondition.condition_field+'"].getTime()-$.fn.dlshouwen.grid.tools.toDate("'+advanceQueryCondition.condition_value+'", "'+advanceQueryCondition.format+'").getTime())>0';
									}
									//数值比较
									if(!(value instanceof Date)&&!isNaN(value)){
										condition += '(parseFloat(record["'+advanceQueryCondition.condition_field+'"]) - parseFloat("'+advanceQueryCondition.condition_value+'"))>0';
									}
									//普通比较
									if(!(value instanceof Date)&&isNaN(value)){
										condition += 'record["'+advanceQueryCondition.condition_field+'"].localeCompare("'+advanceQueryCondition.condition_value+'")>0';
									}
								}
								...
								if(advanceQueryCondition.condition=='9'){
									condition += '!record["'+advanceQueryCondition.condition_field+'"]';
								}
								if(advanceQueryCondition.condition=='10'){
									condition += 'record["'+advanceQueryCondition.condition_field+'"]';
								}
								condition += advanceQueryCondition.right_parentheses;
					...
					if(advanceQueryParameter.advanceQuerySorts&&advanceQueryParameter.advanceQuerySorts.length>0){
						returnDatas = returnDatas.sort(function(record1, record2){
							for(var i=0; i<advanceQueryParameter.advanceQuerySorts.length; i++){
								...
								var value1 = record1[advanceQuerySort.sort_field];
								var value2 = record2[advanceQuerySort.sort_field];
								//如果相同则比较下一个
								if(value1==value2){
									continue;
								}
								//数值比较
								if(!isNaN(value1)&&!isNaN(value2)){
									if(advanceQuerySort.sort_logic=='0'){
										return value1-value2;
									}
									if(advanceQuerySort.sort_logic=='1'){
										return value2-value1;
									}
								}
								//日期比较
								...
								//普通比较
								if(value1!=null&&advanceQuerySort.sort_logic=='0'){
									return value1.localeCompare(value2);
								}
								if(value2!=null&&advanceQuerySort.sort_logic=='1'){
									return value2.localeCompare(value1);
								}
							}
							return 0;
				...
				insertConditionTr : function(){
					...
					content += '	<td id="seq_td_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'"></td>';
					content += '	<td>';
					content += '		<button type="button" id="move_up_condition_tr_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'" class="btn btn-xs btn-primary btn-icon" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.buttons.upTitle+'">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.buttons.up+'</button>';
					content += '		<button type="button" id="move_down_condition_tr_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'" class="btn btn-xs btn-primary btn-icon" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.buttons.downTitle+'">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.buttons.down+'</button>';
					content += '		<button type="button" id="delete_condition_tr_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'" class="btn btn-xs btn-danger btn-icon" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.buttons.deleteTitle+'">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.condition.table.buttons['delete']+'</button>';
					content += '		<input type="hidden" id="data_type_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'" name="data_type_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'" />';
					content += '	</td>';
					content += '	<td>';
					content += '		<input type="text" id="left_parentheses_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'" name="left_parentheses_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'" class="form-control" />';
					content += '	</td>';
					content += '	<td>';
					content += '		<select id="condition_field_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'" name="condition_field_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'" class="form-control">';
					...
					content += '	</td>';
					content += '	<td>';
					content += '		<select id="condition_type_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'" name="condition_type_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'" class="form-control">';
					content += '			<option value=""></option>';
					...
					content += '		</select>';
					content += '	</td>';
					content += '	<td id="condition_value_td_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'"></td>';
					content += '	<td>';
					content += '		<input type="text" id="right_parentheses_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'" name="right_parentheses_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition+'" class="form-control" />';
					content += '	</td>';
					content += '	<td>';
					...
					content += '	</td>';
					content += '</tr>';
					$('#condition_table_'+gridReflectionObj.option.id).append(content);
					//绑定方法-上移
					$('#move_up_condition_tr_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition).click(function(){
						var sequenceCondition = this.id.replace('move_up_condition_tr_'+gridReflectionObj.option.id+'_', '');
						gridReflectionObj.moveUpConditionTr(sequenceCondition);
					});
					//绑定方法-下移
					$('#move_down_condition_tr_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition).click(function(){
						var sequenceCondition = this.id.replace('move_down_condition_tr_'+gridReflectionObj.option.id+'_', '');
						gridReflectionObj.moveDownConditionTr(sequenceCondition);
					});
					//绑定方法-删除
					$('#delete_condition_tr_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition).click(function(){
						var sequenceCondition = this.id.replace('delete_condition_tr_'+gridReflectionObj.option.id+'_', '');
						gridReflectionObj.deleteConditionTr(sequenceCondition);
					});
					//绑定方法-左括号
					$('#left_parentheses_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition).keyup(function(){
						var sequenceCondition = this.id.replace('left_parentheses_'+gridReflectionObj.option.id+'_', '');
						gridReflectionObj.validLeftParentheses(sequenceCondition);
					});
					//绑定方法-字段变更
					$('#condition_field_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition).change(function(){
						var sequenceCondition = this.id.replace('condition_field_'+gridReflectionObj.option.id+'_', '');
						gridReflectionObj.fieldChange(sequenceCondition);
					});
					//绑定方法-条件变更
					$('#condition_type_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition).change(function(){
						var sequenceCondition = this.id.replace('condition_type_'+gridReflectionObj.option.id+'_', '');
						gridReflectionObj.conditionChange(sequenceCondition);
					});
					//绑定方法-右括号
					$('#right_parentheses_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceCondition).keyup(function(){
						var sequenceCondition = this.id.replace('right_parentheses_'+gridReflectionObj.option.id+'_', '');
						gridReflectionObj.validRightParentheses(sequenceCondition);
					});
					var sequenceCondition = gridReflectionObj.sequenceCondition;
				...
				clearConditionTr : function(){
					//定义表格对象映像
					var gridReflectionObj = this;
					$('#condition_table_'+gridReflectionObj.option.id+' tr[id*=tr_'+gridReflectionObj.option.id+'_]').remove();
				},
				//序列号重置
				resetConditionSeq : function(){
					...
					$('#condition_table_'+gridReflectionObj.option.id+' tr[id*=tr_'+gridReflectionObj.option.id+'_]').each(function(){
						...
						$('#condition_table_'+gridReflectionObj.option.id+' #seq_td_'+gridReflectionObj.option.id+'_'+id).html(seq);
						seq++;
					});
				},
				//上移
				moveUpConditionTr : function(id){
					//定义表格对象映像
					var gridReflectionObj = this;
					$('#condition_table_'+gridReflectionObj.option.id+' #tr_'+gridReflectionObj.option.id+'_'+id).prev('#condition_table_'+gridReflectionObj.option.id+' tr[id*=tr_'+gridReflectionObj.option.id+'_]').before($('#condition_table_'+gridReflectionObj.option.id+' #tr_'+gridReflectionObj.option.id+'_'+id));
					gridReflectionObj.resetConditionSeq();
				},
				//下移
				moveDownConditionTr : function(id){
					//定义表格对象映像
					var gridReflectionObj = this;
					$('#condition_table_'+gridReflectionObj.option.id+' #tr_'+gridReflectionObj.option.id+'_'+id).next('#condition_table_'+gridReflectionObj.option.id+' tr[id*=tr_'+gridReflectionObj.option.id+'_]').after($('#condition_table_'+gridReflectionObj.option.id+' #tr_'+gridReflectionObj.option.id+'_'+id));
					gridReflectionObj.resetConditionSeq();
				},
				//删除当前行
				deleteConditionTr : function(id){
					//定义表格对象映像
					var gridReflectionObj = this;
					$('#condition_table_'+gridReflectionObj.option.id+' #tr_'+gridReflectionObj.option.id+'_'+id).remove();
					gridReflectionObj.resetConditionSeq();
				},
				//左括号的验证函数
				validLeftParentheses : function(id){
					...
					var gridReflectionObj = this;
					//执行验证
					var value = $('#left_parentheses_'+gridReflectionObj.option.id+'_'+id).val();
					if(!value.match(/^[\(]*$/g)){
						value = $.fn.dlshouwen.grid.tools.replaceAll(value, '（', '(');
						value = value.replace(/[^\(]/g, '');
						$('#left_parentheses_'+gridReflectionObj.option.id+'_'+id).val(value);
					}
				},
				//列变更触发
				fieldChange : function(id){
					...
					var conditionField = $('#condition_field_'+gridReflectionObj.option.id+'_'+id).val();
					//如果为空或条件一栏为9-10则滞空值栏
					var conditionType = $('#condition_value_'+gridReflectionObj.option.id+'_'+id).val();
					if(conditionField=='' || conditionType=='9' || conditionType=='10'){
						$('#condition_value_td_'+gridReflectionObj.option.id+'_'+id).html('');
						return;
					}
					//获取列对象
					var column = new Object();
					for(var i=0; i<gridReflectionObj.option.columns.length; i++){
						if(gridReflectionObj.option.columns[i].id==conditionField&&gridReflectionObj.option.columns[i].advanceQuery!=false){
							column = gridReflectionObj.option.columns[i];
							break;
						}
					}
					//设置数据类型
					$('#data_type_'+gridReflectionObj.option.id+'_'+id).val(column.type);
					//处理日期类型
					if(column.type=='date'){
						var content = '';
						content += '<div class="input-group">';
						content += '	<input id="condition_value_'+gridReflectionObj.option.id+'_'+id+'" name="condition_value_'+gridReflectionObj.option.id+'_'+id+'" class="form-control" onclick="WdatePicker({dateFmt:\''+$.fn.dlshouwen.grid.tools.replaceAll(column.format, 'h', 'H')+'\'})" />';
						content += '	<div class="input-group-addon"><i class="fa fa-calendar"></i></div>';
						content += '</div>';
						$('#condition_value_td_'+gridReflectionObj.option.id+'_'+id).html(content);
						return;
					}
					//处理codeTable内容
					if(column.codeTable){
						var content = '';
						content += '<select id="condition_value_'+gridReflectionObj.option.id+'_'+id+'" name="condition_value_'+gridReflectionObj.option.id+'_'+id+'" class="form-control">';
						content += '	<option value=""></option>';
						for(var key in column.codeTable){
							content += '<option value="'+key+'">'+column.codeTable[key]+'</option>';
						}
					...
					//其他则为默认文本
					var content = '<input id="condition_value_'+gridReflectionObj.option.id+'_'+id+'" name="condition_value_'+gridReflectionObj.option.id+'_'+id+'" class="form-control" />';
					$('#condition_value_td_'+gridReflectionObj.option.id+'_'+id).html(content);
				},
				//条件触发
				conditionChange : function(id){
					...
					var gridReflectionObj = this;
					var conditionType = $('#condition_type_'+gridReflectionObj.option.id+'_'+id).val();
					if(conditionType == '9' || conditionType == '10'){
						$('#condition_value_td_'+gridReflectionObj.option.id+'_'+id).html('');
					}else{
						var valueHtml = $('#condition_value_td_'+gridReflectionObj.option.id+'_'+id).html();
						if(valueHtml==''){
				...
				validRightParentheses : function(id){
					//定义表格对象映像
					var gridReflectionObj = this;
					var value = $('#right_parentheses_'+gridReflectionObj.option.id+'_'+id).val();
					if(!value.match(/^[\(]*$/g)){
						value = $.fn.dlshouwen.grid.tools.replaceAll(value, '）', ')');
						value = value.replace(/[^\)]/g, '');
						$('#right_parentheses_'+gridReflectionObj.option.id+'_'+id).val(value);
					}
				},
				//排序条件序列号
				sequenceSort : 0,
				...
				insertSortTr : function(){
					...
					content += '	<td id="seq_td_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceSort+'"></td>';
					content += '	<td>';
					content += '		<button type="button" id="move_up_sort_tr_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceSort+'" class="btn btn-xs btn-primary btn-icon" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.buttons.upTitle+'">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.buttons.up+'</button>';
					content += '		<button type="button" id="move_down_sort_tr_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceSort+'" class="btn btn-xs btn-primary btn-icon" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.buttons.downTitle+'">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.buttons.down+'</button>';
					content += '		<button type="button" id="delete_sort_tr_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceSort+'" class="btn btn-xs btn-danger btn-icon" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.buttons.deleteTitle+'">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].advanceQuery.sort.table.buttons['delete']+'</button>';
					content += '	</td>';
					content += '	<td>';
					content += '		<select id="sort_field_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceSort+'" name="sort_field_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceSort+'" class="form-control">';
					content += '			<option value=""></option>';
					for(var i=0; i<gridReflectionObj.option.columns.length; i++){
						var column = gridReflectionObj.option.columns[i];
						if(column.advanceQuery!=false){
					...
					content += '	</td>';
					content += '	<td>';
					content += '		<select id="sort_logic_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceSort+'" name="sort_logic_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceSort+'" class="form-control">';
					content += '			<option value=""></option>';
					...
					content += '	</td>';
					content += '</tr>';
					$('#sort_table_'+gridReflectionObj.option.id).append(content);
					//绑定方法-上移
					$('#move_up_sort_tr_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceSort).click(function(){
						var sequenceSort = this.id.replace('move_up_sort_tr_'+gridReflectionObj.option.id+'_', '');
						gridReflectionObj.moveUpSortTr(sequenceSort);
					});
					//绑定方法-下移
					$('#move_down_sort_tr_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceSort).click(function(){
						var sequenceSort = this.id.replace('move_down_sort_tr_'+gridReflectionObj.option.id+'_', '');
						gridReflectionObj.moveDownSortTr(sequenceSort);
					});
					//绑定方法-删除
					$('#delete_sort_tr_'+gridReflectionObj.option.id+'_'+gridReflectionObj.sequenceSort).click(function(){
						var sequenceSort = this.id.replace('delete_sort_tr_'+gridReflectionObj.option.id+'_', '');
						gridReflectionObj.deleteSortTr(sequenceSort);
					});
					var sequenceSort = gridReflectionObj.sequenceSort;
				...
				clearSortTr : function(){
					//定义表格对象映像
					var gridReflectionObj = this;
					$('#sort_table_'+gridReflectionObj.option.id+' tr[id*=tr_'+gridReflectionObj.option.id+'_]').remove();
				},
				//序列号重置
				resetSortSeq : function(){
					...
					$('#sort_table_'+gridReflectionObj.option.id+' tr[id*=tr_'+gridReflectionObj.option.id+'_]').each(function(){
						var id = this.id.replace('tr_'+gridReflectionObj.option.id+'_', '');
						$('#sort_table_'+gridReflectionObj.option.id+' #seq_td_'+gridReflectionObj.option.id+'_'+id).html(seq);
						seq++;
					});
				},
				//上移
				moveUpSortTr : function(id){
					//定义表格对象映像
					var gridReflectionObj = this;
					$('#sort_table_'+gridReflectionObj.option.id+' #tr_'+gridReflectionObj.option.id+'_'+id).prev('#sort_table_'+gridReflectionObj.option.id+' tr[id*=tr_'+gridReflectionObj.option.id+'_]').before($('#sort_table_'+gridReflectionObj.option.id+' #tr_'+gridReflectionObj.option.id+'_'+id));
					gridReflectionObj.resetSortSeq();
				},
				//下移
				moveDownSortTr : function(id){
					//定义表格对象映像
					var gridReflectionObj = this;
					$('#sort_table_'+gridReflectionObj.option.id+' #tr_'+gridReflectionObj.option.id+'_'+id).next('#sort_table_'+gridReflectionObj.option.id+' tr[id*=tr_'+gridReflectionObj.option.id+'_]').after($('#sort_table_'+gridReflectionObj.option.id+' #tr_'+gridReflectionObj.option.id+'_'+id));
					gridReflectionObj.resetSortSeq();
				},
				//删除当前行
				deleteSortTr : function(id){
					//定义表格对象映像
					var gridReflectionObj = this;
					$('#sort_table_'+gridReflectionObj.option.id+' #tr_'+gridReflectionObj.option.id+'_'+id).remove();
					gridReflectionObj.resetSortSeq();
				},
				/**
				 * 打印相关
				...
				print : function(){
					...
					for(var i=0; i<gridReflectionObj.option.columns.length; i++){
						...
						content += '			<td>';
						content += '				<button type="button" class="btn btn-xs btn-default" dataId="'+i+'" id="dlshouwen_grid_print_up_'+gridReflectionObj.option.id+'_'+i+'" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].print.table.buttons.upTitle+'">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].print.table.buttons.up+'</button>';
						content += '				<button type="button" class="btn btn-xs btn-default" dataId="'+i+'" id="dlshouwen_grid_print_down_'+gridReflectionObj.option.id+'_'+i+'" title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].print.table.buttons.downTitle+'">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].print.table.buttons.down+'</button>';
						content += '			</td>';
						content += '		</tr>';
					}
					content += '		</tbody>';
					...
					$('#dlshouwen_grid_print_check_'+gridReflectionObj.option.id).click(function(){
						$('input[id*=dlshouwen_grid_print_check_'+gridReflectionObj.option.id+'_]').attr('checked', this.checked);
					});
					//绑定上移方法
				...
				exportFile : function(exportType){
					...
					content += '<div class="form-export modal-body form-horizontal">';
					content += '	<div class="form-group">';
					...
					$('#dlshouwen_grid_export_check_'+exportType+'_'+gridReflectionObj.option.id).click(function(){
						$('input[id*=dlshouwen_grid_export_check_'+exportType+'_'+gridReflectionObj.option.id+'_]').attr('checked', this.checked);
					});
					//绑定上移方法
		...
		tools : {
			...
			//创建一个模态窗口（开始）
			getWindowStart : function(id, title, modalType){
				var modalTypeContent = '';
				if(modalType){
					if(modalType=='lg') modalTypeContent = 'modal-lg';
					if(modalType=='sm') modalTypeContent = 'modal-sm';
				}
				var content = '';
				content += '<div class="modal fade dlshouwen-grid-modal" id="'+id+'_modal" tabindex="-1" role="dialog" aria-labelledby="'+id+'ModalLabel" aria-hidden="true">';
				...
				content += '				<h5 class="modal-title" id="'+id+'ModalLabel">'+title+'</h5>';
				content += '			</div>';
				return content;
			},
			//创建一个模态窗口（结束）
...
(function($) {
	$.fn.dlshouwen.grid.defaultOptions = {
		grid : {
			...
			tableStyle : '',
			tableClass : 'table table-bordered table-hover table-responsive table-condensed',
			showHeader : true,
			gridContainer : 'grid_container',
			toolbarContainer : 'grid_toolbar_container',
			tools : 'refresh|fastQuery|advanceQuery|export[excel,csv,pdf,txt]|print',
			exportFileName : 'datas',
			exportURL : path + '/core/base/extra/grid/export',
			pageSize : 20,
