...
function getFileList(filePath){
	...
	var url = path+'/core/server/image/list';
...
function constructImages(){
	...
	if(showType=='0'){
		...
		content += '</table>';
		$('#show_images_container').html(content);
	}else{
		var content = '';
		for(var i=0; i<showFiles.length; i++){
			var file = showFiles[i];
			...
			if(file.is_dir=='1'){
				...
				content += '			<img width="100%" height="53" src="'+(previewPath+nowPath+file.full_name)+'" />';
			}
...
function bindMethod(){
	...
	if(isSelect=='1'){
		$('.type-file').dblclick(function(){
			...
			sw.getTopWindow(window).model_args['selectImageReturnValue'] = result;
 ...
 */
function addFolder(){
	var url = path+'/core/server/image/add';
	var params = new Object();
...
function modifyFileName(){
	...
	var url = path+'/core/server/image/rename';
	var params = new Object();
	params.nowPath = nowPath;
...
function deleteFile(){
	...
	var url = path+'/core/server/image/delete';
	var params = new Object();
	params.nowPath = nowPath;
	params.nowFile = nowFile;
	sw.showProcessBar();
...
function selectImage(){
	if($('.file-single-select').hasClass('type-dir')){
		sw.toast('您不能选择文件夹，请选择图片。', 'warning', 3000);
		return;
	}
	var image = $('.file-single-select').attr('file_full_name');
	modalCallback(image);
	closeModal('selectImage', parent);
}
