 ...
 * @example sw.ajaxSubmit(url, params);
 */
DLShouWen.prototype.ajaxSubmitForForm = function(url, params, fileId, successFunc, errorFunc){
	$.ajaxFileUpload({
		type:'post',
		url:url,
		data:params,
...
DLShouWen.prototype.ajaxSuccessCallback = function(data, successFunc, warningFunc, errorFunc){
	try{
		if(data.success){
			sw.toast(data.successMessage, 'success', 1500);
			if(successFunc)
				try{successFunc(data);}catch(e){};
		}
		if(data.warning){
			sw.toast(data.warningMessage, 'warning', 1500);
			if(warningFunc)
				try{warningFunc(data);}catch(e){};
		}
		if(data.error){
			sw.toast(data.errorMessage, 'error', 1500);
			if(errorFunc)
				try{errorFunc(data);}catch(e){};
		}
                if(data.extParam != null && data.extParam.URL != null) {
                    //获取当前网址
...
//                    setTimeout(function(){
                        window.location.href = localhostPaht+projectName+'/'+data.extParam.URL;
//                    },1000);
...
function change(data){
    if(data.warningMessage != null){
        data.warningMessage = data.warningMessage.replaceAll("&lt;","<");
        data.warningMessage = data.warningMessage.replaceAll("/&gt;","/>");
    }else if(data.successMessage != null){
        data.successMessage = data.successMessage.replaceAll("&lt;","<");
        data.successMessage = data.successMessage.replaceAll("/&gt;","/>");
    }else if(data.errorMessage != null){
        data.errorMessage = data.errorMessage.replaceAll("&lt;","<");
        data.errorMessage = data.errorMessage.replaceAll("/&gt;","/>");
    }
